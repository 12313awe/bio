...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  // Detect browser language and use it as default when available
  ...
  // Memoize the translation function to avoid recreating it on each render
  const t = useMemo(() => {
    return (key: string): string => {
      // Type safety: Check if the language exists in translations
      if (!translations[language]) {
        console.warn(`Missing translations for language: ${language}`);
        return key;
      }
      
      // Type safety: Check if the key exists in the selected language
