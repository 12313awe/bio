...
// Kapsamlı Markdown renderer fonksiyonu
const renderMarkdown = (text: string) => {
  if (!text) return '';
  
  // Önce kod bloklarını koruyalım
  ...
  // Satır içi formatlama
  return result
    .replace(/\*\*\*(.*?)\*\*\*/g, '<strong><em>$1</em></strong>') // ***kalın ve italik***
    .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') // **kalın**
    .replace(/\*(.*?)\*/g, '<em>$1</em>') // *italik*
    .replace(/~~(.*?)~~/g, '<s>$1</s>') // ~~üstü çizili~~
    .replace(/`([^`]+)`/g, '<code class="bg-gray-100 px-1 py-0.5 rounded text-sm font-mono">$1</code>') // `kod`
    .replace(/!\[(.*?)\]\((.*?)\)/g, '<img src="$2" alt="$1" class="my-2 max-w-full h-auto rounded" />') // resim
    .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-venice-coral hover:underline" target="_blank" rel="noopener noreferrer">$1</a>') // link
    .replace(/\n\s*\n/g, '</p><p>') // Paragraflar
    .replace(/\n/g, '<br />') // Yeni satırlar
    .replace(/<p>(.*?)<\/p>/g, (_, p1) => p1.startsWith('<') ? p1 : `<p class="my-2">${p1}</p>`); // Paragraf stilleri
};
...
// Memoized message formatter component to prevent unnecessary re-renders
